...
def track_order(request, id=None):
    ...
    mydata = {'users': users, 'd_name': d_name, 'd_phone_number': d_phone_number, 'd_district': d_district,
              'd_pincode': d_pincode, 'd_address': d_address, 'd_pickup_time': d_pickup_time, 'date': date_p,
              'order_id': od_id}
...
def searchfield(request):
    ...
    if cat == 'all':
        ...
        data = product.objects.filter(product_name=name,
                                      product_category__in=all_data)  # filedsname__in  use for filter multiple data
    else:
...
#     if request.method =='POST':
#         forms = Form_feedback(request.POST)
#         if forms.is_valid():
#             forms.save()
#     else:
#         forms = Form_feedback()
